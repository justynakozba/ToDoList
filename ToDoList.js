...
function getAllTasks() {
  ...
  request.onload = function() {
    ...
    if (request.status >= 200 && request.status < 400) {
      ...
      window.alert(
        "Failed with error: " + request.status + ". Failed to get total pages."
      );
    }
  };
  request.send();
...
function newElement() {
  ...
  if (title == "") {
    ...
    window.alert(
      "The deadline format is invalid, Please set date in YYYY-MM-DD format"
    );
  } else {
...
function deleteSelected() {
  ...
  for (var i = 0; i < length; ) {
    if (myNodelist[i].childNodes[2].checked) {
      console.log(
        "Deleting entry with ID" + myNodelist[i].id + " end row: " + i
      );
